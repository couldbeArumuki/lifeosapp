...
import { Plus, Moon, Smile, Trash2 } from 'lucide-react';
...
const MSTrace = () => {
  ...
  const logLabel = { mood: 'Mood', sleep: 'Sleep' };

  return (
    <div className="space-y-6 animate-slide-up">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-heading font-bold text-text-dark dark:text-text-light">M&amp;S Trace</h1>
      ...
      <Modal isOpen={showMoodModal} onClose={() => setShowMoodModal(false)} title="Log Mood" size="sm">
        <form onSubmit={handleMoodSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {MOODS.map(m => (
                <button
                  key={m.key}
                  type="button"
                  onClick={() => setMoodField('mood', m.key)}
                  className={`flex flex-col items-center gap-1 p-2 rounded-xl border-2 transition-all text-xs font-medium ${moodForm.mood === m.key ? 'border-primary bg-primary/10 text-primary' : 'border-gray-100 dark:border-white/10 text-gray-500 hover:border-gray-200'}`}
                >
          ...
          <Input label="Notes (optional)" placeholder="What's on your mind?" value={moodForm.notes} onChange={e => setMoodField('notes', e.target.value)} />
      ...
      {/* Log Sleep Modal */}
      <Modal isOpen={showSleepModal} onClose={() => { setShowSleepModal(false); setSleepErrors({}); }} title="Log Sleep" size="sm">
        <form onSubmit={handleSleepSubmit} className="space-y-4">
          ...
          <Input label="Notes (optional)" placeholder="How did you sleep?" value={sleepForm.notes} onChange={e => setSleepField('notes', e.target.value)} />
          <div className="flex gap-3 pt-2">
